...
// VybeMatch has no billing, subscription, or in-app-purchase system yet --
...
export default async function PaymentsPage() {
  ...
  return (
    <div>
      <PageHeader title="Payments & Subscriptions" description="Not yet applicable -- VybeMatch has no billing system yet." />
      <div className="p-8">
        <EmptyState
          title="No payments data to show"
          description="There is no Subscription, Payment, or Invoice model in the product yet -- VybeMatch doesn't charge users today. Once a billing provider (e.g. Razorpay/Stripe) and a subscription model are added to the schema, this page is where plan status, transaction history, refunds/chargebacks, and revenue reporting (payments.view / reporting.export) would live, wired the same way every other module here is: real Prisma queries, permission-gated, audit-logged."
        />
      </div>
    </div>
  );
